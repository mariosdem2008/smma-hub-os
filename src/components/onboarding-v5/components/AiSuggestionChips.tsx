...
// Shared by Onboarding V5 sections (moved from legacy onboarding-v4)
// ============================================================================
...
export function AiSuggestionChips({
  ...
  const handleChipClick = (suggestion: Suggestion) => {
    if (multiSelect) {
      ...
      if (isSelected) {
        onSelect(
          selected.filter((s) => s !== suggestion.label),
          'user_selected'
        );
    ...
    <div className={cn('space-y-4', className)}>
          ...
          return (
            ...
            >
              ...
              {suggestion.confidence !== undefined &&
                suggestion.confidence >= 80 && <Sparkles className="ml-1 h-3 w-3" />}
            </Badge>
          );
        })}
      ...
      {multiSelect && (
        <p className="text-xs text-muted-foreground">
          {selected.length} of{' '}
          {minSelect === maxSelect ? minSelect : `${minSelect}-${maxSelect}`} selected
        </p>
      )}
        ...
        <div className="pt-2">
          {showCustomInput ? (
            <div className="flex gap-2">
              ...
              />
              <Button type="button" size="sm" onClick={handleAddCustom} disabled={!customValue.trim()}>
                Add
              </Button>
              <Button
                type="button"
...
export function SingleSelectChips(props: SingleSelectChipsProps) {
  const { options, className, allowCustom = false, customPlaceholder = 'Add a custom option' } = props;
  ...
  const [showCustomInput, setShowCustomInput] = useState(false);
  const [customValue, setCustomValue] = useState('');
  const isCustomSelected = !!selectedValue && !options.some((option) => option.id === selectedValue);
  const handleAddCustom = () => {
    if (!customValue.trim()) return;
  ...
  return (
    <div className={cn('space-y-2', className)}>
      ...
      <p className="text-xs text-muted-foreground pt-2">
        Select {minSelections === maxSelections ? minSelections : `${minSelections}-${maxSelections}`} options (
