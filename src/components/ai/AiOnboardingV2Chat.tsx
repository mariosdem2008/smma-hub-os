...
import { ensureAgencyExists } from "@/data";
...
export function AiOnboardingV2Chat({ onboardingType, clientId }: Props) {
  ...
  const [agencySetupName, setAgencySetupName] = useState("");
  const [creatingAgency, setCreatingAgency] = useState(false);
  const [agencySetupError, setAgencySetupError] = useState<string | null>(null);
  ...
  const handleCreateAgency = async () => {
    if (!agencySetupName.trim()) {
      setAgencySetupError("Agency name is required.");
      return;
    }
    setAgencySetupError(null);
    setCreatingAgency(true);
    try {
      const newAgencyId = await ensureAgencyExists(agencySetupName.trim());
      setAgencyId(newAgencyId);
    } catch (error: any) {
      setAgencySetupError(error?.message || "Failed to create agency.");
    } finally {
      setCreatingAgency(false);
    }
  };

  if (!agencyId && onboardingType === "agency") {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Create Your Agency First</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Enter your agency name to start AI onboarding.
          </p>
          <Input
            value={agencySetupName}
            onChange={(event) => setAgencySetupName(event.target.value)}
            placeholder="Agency name"
          />
          {agencySetupError && (
            <p className="text-sm text-destructive">{agencySetupError}</p>
          )}
          <Button onClick={handleCreateAgency} disabled={creatingAgency}>
            {creatingAgency ? "Creating..." : "Create Agency & Start"}
          </Button>
        </CardContent>
      </Card>
    );
  ...
  if (!agencyId) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Client Not Ready</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
            We could not find an agency for this client. Ask an admin to check the client setup.
          </p>
        </CardContent>
      </Card>
    );
